...
var unshiftAll=(val)=>{
    val[0] && val[0].value != 'All' &&val.unshift({label: "All",value: "All",selected: 0});
    return val
};
var CascadedSelect = React.createClass({
    getInitialState(){
        return {
            ...
            optionsState: unshiftAll(this.props.options) || null, //所有的options，这里不需要key因为在渲染show的时候已经产生了动态的坐标
    ...
    getDefaultProps() {
        return {
            ...
            style: {},
            placeholder: '',
            //selected: [],//  已废弃（初始传过来的值没想到好的格式嵌入options树）：初始已选择的部分,[[1, 2], [2, 'All']]第二个option下的第三个option 和 第三个option下的所有option
            onFocus(){
            },
            onBlur(){
            },
    ...
    // ### 方法
    nodeSelectAllMap(Parent){
        return Parent.map((item)=> {
            if (item.children) {
                item.selected = item.children.length * 1000;
                this.nodeSelectAllMap(item.children)
            } else item.selected = 1000;
        })
    },
    nodeCancelAllMap(Parent){
        return Parent.map((item)=> {
            if (item.children) {
                item.selected = 0;
    ...
    findOptionNode(route) {
        var option = this.state.optionsState,
    ...
    getAllChildrenValue(parent, valueArray = []){
    ...
    search(value = this.refs.input.value, Parent = this.state.optionsState, key = [], prevLabel = '', result = []){
        Parent.map((item, index)=> {
            if (item.label.toLowerCase().indexOf(value.toLowerCase()) != -1) {
                result.push({
                    ...
                    selectedType: item.selected > 0 ? ((item.children ? (item.selected == item.children.length * 1000) : (item.selected == 1000)) ? 1 : 2) : 0
                })
            }
            if (item.children) {
    ...
    values(){
        ...
        this.state.valueKey.map((item)=> {
            ...
            (typeof option).toLowerCase() == 'object' ? map(option, (item)=> {
                result.push(item)
            }) : result.push(option)
        });
        return result;
    ...
    // ### 事件函数
    selectChange(coordinate, isOptions, key){
        var {show,optionsState,valueKey,valuesState,showSearch} = this.state;
        // state改变需要加入动画
        ...
        var setShowSearch = ()=> showSearch && this.setState({searchState: this.search()});
        // 拿到正确的option以及向上整个引用数组，此处不要用map循环，不能打断，好深的坑
        ...
        let shouldSmall = false,
            ...
            shouldBreak;
        // 更改option树
        if (isOptions ? (optionRefer.selected != optionChildren.length * 1000) : (optionRefer.selected === 0 || optionRefer.selected === undefined)) {
            ...
            for (i = routeArray.length - 1; i >= 0; i--) {
                ...
                if (i != routeArray.length - 1) {
                    ...
                    if (shouldSmall) {
                        ...
                        shouldBreak = routeTemp.selected;
                        routeTemp.selected = computeFn(routeTemp.selected, true, 1000 - deleteNum);
                    }
                    // （当前项没有全选）？（父级应该加小于1000的数）：（父级应该加1000了，记录当前是否选择了子项传入父级）
                    (routeTemp.selected != routeTemp.children.length * 1000) ? (shouldSmall = true) : (deleteNum = 1);
                }
                else {
                    if (isOptions) {
                        routeTemp.selected == undefined && (routeTemp.selected = 0);
                        (routeTemp.selected != 0) && (deleteNum = 1);
                        routeTemp.selected = routeTemp.children.length * 1000;
                    } else {
                        routeTemp.selected = computeFn(routeTemp.selected, true, 1000)
                    }
                }
            ...
            for (i = routeArray.length - 1; i >= 0; i--) {
                ...
                if (i != routeArray.length - 1) {
                    if (shouldSmall) {
                        ...
                        routeTemp.selected = computeFn(routeTemp.selected, false, 1000 - deleteNum);
                        shouldBreak = routeTemp.selected > 0
                    }
                    // （当前项点击前没有全选）？（父级应该加小于1000的数）：（父级应该加1000了，记录当前是否选择了子项传入父级）
                    (parseInt(routeTemp.selected / 1000) != (routeTemp.children.length - 1)) ? (shouldSmall = true) : (deleteNum = 1);
                }
                else {
                    isOptions ? (routeTemp.selected = 0) : (routeTemp.selected = computeFn(routeTemp.selected, false, 1000));
                }
        ...
        optionsState.map((item, i)=> {
            (i != 0 && item.children) && (isAll += item.selected == item.children.length * 1000);
        });
        (isAll == optionsState.length - 1) ? (optionsState[0].selected = 1000) : (optionsState[0].selected = 0);
        // 生成values
        if (isOptions ? (optionRefer.selected == optionRefer.children.length * 1000) : (optionRefer.selected == 1000)) {//全选
            if (isAll == optionsState.length - 1) {
                ...
                this.setState({valuesState: valueTemp, valueKey: keyTemp});
                setShowSearch();
            ...
            for (index = 0; index < routeArray.length; index++) {
                if (routeArray[index].children && routeArray[index].selected == routeArray[index].children.length * 1000)break;
            ...
            this.setState({valuesState: valuesStateTemp, valueKey: valueKeyTemp});
        ...
        function findChildrenValues(item, key, prevLabel = '', valuesArray = [], keysArray = []) {
            ...
            item.map((item, index)=> {
                if ((item.children && item.selected == item.children.length * 1000) || (!item.children && item.selected == 1000)) {
    ...
    componentWillReceiveProps(nextProps){
        this.setState({optionsState:unshiftAll(nextProps.options)});
    },
    render(){
                    ...
                    return (
                        <ul className="ant-cascader-menu" key={`list${indexShow}`}>
                            {optionsTemp.map((item, index)=> {
                                ...
                                if (item.children) {
                                    var type = item.selected > 0 ? (item.selected == item.children.length * 1000 ? 1 : 2) : 0;//0未选1全选2部分选择
                                    return <li
                                    ...
                                    return <li
                                        className={classnames("ant-cascader-menu-item",{['selected']:item.selected==1000})}
                                        key={item.value}
                                ...
                                valuesState.map((item, index)=>
                                    <li className={classnames("ant-select-selection__choice",{["zoom-leave"]:item.state===0,["zoom-enter"]:item.state===1})}
                                        ...
                                        <span className="ant-select-selection__choice__remove"
                                              onClick={this.selectChange.bind(this,valueKey[index],true,index)}></span>
                                    </li>
                                )
                            }
                    ...
                    <ul className="ant-select-dropdown-menu ant-select-dropdown-menu-vertical  ant-select-dropdown-menu-root">
                        {
                            searchState.length > 0 ? searchState.map((item, index)=>
                                <li className={classnames("ant-select-dropdown-menu-item",{['ant-select-dropdown-menu-item-selected']:item.selectedType==1,['selected_some']:item.selectedType==2})}
                                    title={item.label} key={item.value}
                                    onClick={this.selectChange.bind(this,item.key,true,index)}>
                                    {item.label}
                                </li>) :
